...
/* Expression kind codes for op_preprocess_expression */
...
/* Local functions */
static Node *op_preprocess_expression(PlannerInfo *root, Node *expr, int kind);
static void op_preprocess_qual_conditions(PlannerInfo *root, Node *jtnode);
				 ...
				 bool hasRecursion, double tuple_fraction)
{
	 ...
	 * because op_preprocess_expression will reduce a constant-true condition to
	...
	parse->targetList = (List *)
		op_preprocess_expression(root, (Node *) parse->targetList,
							  EXPRKIND_TARGET);
	...
	foreach(l, parse->withCheckOptions)
	{
		...
		wco->qual = op_preprocess_expression(root, wco->qual,
										  EXPRKIND_QUAL);
		if (wco->qual != NULL)
	...
	parse->returningList = (List *)
		op_preprocess_expression(root, (Node *) parse->returningList,
							  EXPRKIND_TARGET);

	op_preprocess_qual_conditions(root, (Node *) parse->jointree);

	parse->havingQual = op_preprocess_expression(root, parse->havingQual,
											  EXPRKIND_QUAL);
	...
	foreach(l, parse->windowClause)
	{
		...
		wc->startOffset = op_preprocess_expression(root, wc->startOffset,
												EXPRKIND_LIMIT);
		wc->endOffset = op_preprocess_expression(root, wc->endOffset,
											  EXPRKIND_LIMIT);
	}

	parse->limitOffset = op_preprocess_expression(root, parse->limitOffset,
											   EXPRKIND_LIMIT);
	parse->limitCount = op_preprocess_expression(root, parse->limitCount,
											  EXPRKIND_LIMIT);

	if (parse->onConflict)
	{
		parse->onConflict->arbiterElems = (List *)
			op_preprocess_expression(root,
	...
	root->append_rel_list = (List *)
		op_preprocess_expression(root, (Node *) root->append_rel_list,
							  EXPRKIND_APPINFO);
	...
	foreach(l, parse->rtable)
	{
		...
		else if (rte->rtekind == RTE_FUNCTION)
		{
			...
			rte->functions = (List *)
				op_preprocess_expression(root, (Node *) rte->functions, kind);
		}
		else if (rte->rtekind == RTE_TABLEFUNC)
		{
			...
			rte->tablefunc = (TableFunc *)
				op_preprocess_expression(root, (Node *) rte->tablefunc, kind);
		}
		else if (rte->rtekind == RTE_VALUES)
		{
			...
			rte->values_lists = (List *)
				op_preprocess_expression(root, (Node *) rte->values_lists, kind);
		}
		...
		{
			lfirst(lcsq) = op_preprocess_expression(root,
												 (Node *) lfirst(lcsq),
												 EXPRKIND_QUAL);
		}
	}
 ...
 */
static Node *
op_preprocess_expression(PlannerInfo *root, Node *expr, int kind)
{
	/*
	 ...
	 */
	elog(WARNING, "%s kind %d", __FUNCTION__, kind);
	if (expr == NULL)
		return NULL;
 ...
 */
static void
op_preprocess_qual_conditions(PlannerInfo *root, Node *jtnode)
{
	elog(WARNING,"call %s", __FUNCTION__);
	...
	else if (IsA(jtnode, FromExpr))
	{
		...
		foreach(l, f->fromlist)
			op_preprocess_qual_conditions(root, lfirst(l));

		f->quals = op_preprocess_expression(root, f->quals, EXPRKIND_QUAL);
	}
	else if (IsA(jtnode, JoinExpr))
	{
		JoinExpr   *j = (JoinExpr *) jtnode;

		op_preprocess_qual_conditions(root, j->larg);
		op_preprocess_qual_conditions(root, j->rarg);

		j->quals = op_preprocess_expression(root, j->quals, EXPRKIND_QUAL);
	}
	else
...
{
	return (Expr *) op_preprocess_expression(root, (Node *) expr, EXPRKIND_PHV);
}
